...
class Unpool(nn.Module):
    ...
    def __init__(self, num_channels, device, stride=2):
...
class UpConv(Decoder):
    ...
    def upconv_module(self, in_channels, device):
        # UpConv module: unpool -> 5*5 conv -> batchnorm -> ReLU
        upconv = nn.Sequential(collections.OrderedDict([
            ('unpool', Unpool(in_channels, device)),
    ...
    def __init__(self, in_channels, device):
        ...
        self.layer2 = self.upconv_module(in_channels // 2, device)
        self.layer3 = self.upconv_module(in_channels // 4, device)
        self.layer4 = self.upconv_module(in_channels // 8, device)
...
class UpProj(Decoder):
    ...
    class UpProjModule(nn.Module):
        ...
        def __init__(self, in_channels, device):
            ...
            self.unpool = Unpool(in_channels, device)
    ...
    def __init__(self, in_channels, device):
        ...
        self.layer1 = self.UpProjModule(in_channels, device)
        self.layer2 = self.UpProjModule(in_channels // 2, device)
        self.layer3 = self.UpProjModule(in_channels // 4, device)
        self.layer4 = self.UpProjModule(in_channels // 8, device)
...
def choose_decoder(decoder, in_channels, device):
    if decoder[:6].lower() == 'deconv':
        assert len(decoder) == 7
        kernel_size = int(decoder[6])
        return DeConv(in_channels, kernel_size)
    elif decoder.lower() == "upproj":
        return UpProj(in_channels, device)
    elif decoder.lower() == "upconv":
        return UpConv(in_channels, device)
    elif decoder.lower() == "fasterupproj":
        return FasterUpProj(in_channels)
    else:
        assert False, "invalid option for decoder: {}".format(decoder)


class FCRN_ResNet50v2(nn.Module):
    def __init__(self, device, decoder = 'upproj', pretrained=True):


        super(FCRN_ResNet50v2, self).__init__()
        ...
        self.upSample = choose_decoder(decoder, num_channels // 2, device)
...
#     device = 'cuda:2'
#     model = FCRN_ResNet50v2(device, decoder = 'fasterupproj')
#     model = model.to(device)
#     model.eval()
#     image = torch.randn(8, 3, 228, 304)
